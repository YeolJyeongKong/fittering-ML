...
class Beta2Measurements:
    def __init__(self, device):
        ...
        self.smpl_model = SMPL(
            model_path=paths.SMPL_MODEL_DIR,
            J_REGRESSOR_EXTRA_PATH=paths.J_REGRESSOR_EXTRA_PATH,
            COCOPLUS_REGRESSOR_PATH=paths.COCOPLUS_REGRESSOR_PATH,
            H36M_REGRESSOR_PATH=paths.H36M_REGRESSOR_PATH,
        ).to(self.device)
    ...
    def predict(self, betas):
        ...
        smpl_output = self.smpl_model(
            body_pose=pose_rotmats,
            global_orient=glob_rotmats,
            betas=betas,
            pose2rot=False,
        )
        ...
        for i in range(batch_size):
            measurements = MeasureVerts.verts2meas(
                smpl_output.vertices[i], smpl_output.joints[i]
            )
