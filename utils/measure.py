...
class MeasureVerts:
    ...
    @classmethod
    def create_landmarks_plot(cls, landmark_names: List[str], verts: np.ndarray
                                ) -> List[plotly.graph_objs.Scatter3d]:
        plots = []

        landmark_colors = dict(zip(cls.landmarks.keys(),
    ...
    @classmethod
    def create_measurement_length_plot(cls, measurement_name: str, verts: np.ndarray, measurements,
                                       color: str):
        measurement_landmarks_inds = cls.length_definitions[measurement_name]
    ...
    @classmethod
    def create_measurement_circumference_plot(cls, measurement_name: str, verts: np.ndarray, joints, measurements,
                                              color: str):
        '''
    ...
    def visualize(cls, verts, joints, measurements, title="Measurement visualization"):
        ...
        if "LANDMARKS" in cls.cached_visualizations.keys():
            ...
            cls.cached_visualizations["LANDMARKS"] = {landmark_names[i]: landmarks_plot[i] 
                                                    for i in range(len(landmark_names))}
            fig.add_traces(landmarks_plot)
        

        # visualize measurements
        measurement_colors = dict(zip(cls.measurement_types.keys(),
                                  px.colors.qualitative.Alphabet))
        ...
        for m_name in measurement_names:
            ...
            if cls.measurement_types[m_name] == MeasurementType().LENGTH:
                ...
                if m_name in cls.cached_visualizations["LENGTHS"]:
                    ...
                    measurement_plot = cls.create_measurement_length_plot(measurement_name=m_name,
                                                                        verts=verts, measurements=measurements,
                ...
                if m_name in cls.cached_visualizations["CIRCUMFERENCES"]:
                    measurement_plot = cls.cached_visualizations["CIRCUMFERENCES"][m_name]
                else:
                    measurement_plot = cls.create_measurement_circumference_plot(measurement_name=m_name,
                                                                                verts=verts,
                                                                                joints=joints, measurements=measurements,
                                                                                color=measurement_colors[m_name])
                    cls.cached_visualizations["CIRCUMFERENCES"][m_name] = measurement_plot
