...
class RealDataPredictLogger(Callback):
    def __init__(self, real_user_dir):
        ...
        for idx, user_dir in enumerate(glob.glob(os.path.join(real_user_dir, "*"))):
            front = transform(cv2.imread(os.path.join(user_dir, 'front.jpg'), cv2.IMREAD_GRAYSCALE))
            side = transform(cv2.imread(os.path.join(user_dir, 'side.jpg'), cv2.IMREAD_GRAYSCALE))
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        front, side = self.front_tensor.to(pl_module.device), self.side_tensor.to(pl_module.device)
